...
export default function YourBidCard(props: {
    ...
    return (
        <A class="flex w-3/5 h-48  rounded-lg rounded-xl hover:bg-[#0F1629] justify-around border-2 gap-1 align-center"
           href={`/products/${props.productId}`}>
            ...
            <div class="flex flex-col justify-center items-center gap-1 basis-1/4 m-2 pb-3">
                <div
                    class={`font-bold text-right text-6xl ${(props.yourBid >= props.currentBid) ? 'text-[#007e1c]' : 'text-[#921111]'} m-1 w-11/12`}>B{props.yourBid}</div>
                <div class=" text-right text-xl text-[#7F8EA3] w-11/12 whitespace-nowrap">1/1/1970 6:40:56 AM</div>
                <div
                    class=" text-right text-xl text-[#7F8EA3] w-11/12">{(props.yourBid >= props.currentBid) ? 'Outbids: ' : 'Outbid By: '}
                    {Math.abs(props.yourBid - props.currentBid)}</div>
            </div>
