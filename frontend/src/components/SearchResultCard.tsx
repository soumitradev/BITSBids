...
export default function ProductCard({
    ...
    return (
        <A
            class="flex w-10/12 h-80 rounded-lg border-2 gap-1 bg-slate-800"
            href={`/products/${id}`}
        >
            <img
                class="p-4 m-0 w-1/3 rounded-lg object-cover"
                src={imgUrl}
                alt="smtg"
            />
            <div class="flex flex-col gap-2 align-left p-4 ">
                <span class="font-bold text-3xl">{name}</span>
                <span class="text-gray-300 font-light">{description.split(' ').slice(0, 25).join(" ")}</span>
                <Progress
                    value={
                        100 -
                ...
                <A class="bg-white text-black border-white border-2 rounded-lg w-24 p-2 text-center"
                   href={`/products/${id}`}>Bid Now</A>
